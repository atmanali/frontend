import { Avatar, Box, Button, Paper, TextField, Toolbar, Typography } from '@mui/material';
...
const regexArray = [/(?=.*\d)/, /(?=.*[a-z])/, /(?=.*[A-Z])/, /(?=.*\W)/, /.{12,}/]
const totalRegex = RegExp(regexArray.map(regex => regex.toString()).join('').replaceAll('/',''))
export default function Login() {
    const router = useRouter();
    const [passwordType, setPasswordType] = useState<'password'|''>('password');
    const [passwordValidity, setPasswordValidity] = useState<0|20|40|60|80|100>(0);
    ...
    const isValidPassword = (event: ChangeEvent<HTMLTextAreaElement | HTMLInputElement>)=>{
        let validity: 0 | 20 | 40 | 60 | 80 | 100 = 0;
        regexArray.map((regex) => validity += regex.test(event.target.value) ? 20 : 0);
        ...
        <>
            ...
            >
                <Toolbar sx={{ alignSelf: 'center' }} >
                    ...
                    <Avatar sx={{backgroundColor: isConnected ? 'success.light' : 'secondary.light'}}>
                    ...
                    InputProps={{
                        endAdornment: (
                            <Box
                                ...
                                sx={{ display: 'flex', opacity: .4, '&:hover': { cursor: 'pointer' } }}
                            >
